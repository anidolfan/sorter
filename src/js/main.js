...
function init() {
  ...
  document.querySelector('.finished.tweet.button').addEventListener('click', tweet);
  ...
  document.addEventListener('keypress', (ev) => {
    ...
    if (timestamp && !timeTaken && !loading && choices.length === battleNo - 1) {
      switch (ev.key) {
        ...
        case 'b': case 'ArrowLeft': pick('left'); break;
        case 'm': case 'ArrowRight': pick('right'); break;
        case 'n': case '1': case 'ArrowUp': pick('tie'); break;
  ...
  //Total images to display 100 @me
  for (let i = 100; i >= 1; i--) {
    const select = document.createElement('option');
    select.value = i;
    select.text = i;
    //Select default value @me
    if (i === 50) { select.selected = 'selected'; }
...
function start() {
  ...
  if (characterDataToSort.length < 2) {
    alert('Please select one or multiple groups');
    return;
  }
 ...
 */
//Number of images to display. Defaults to 50. @me
function result(imageNum = 50) {
  ...
  //const timeStr = `This sorter was completed on ${new Date(timestamp + timeTaken).toString()} and took ${msToReadableTime(timeTaken)}. <a href="${location.protocol}//${sorterURL}">Do another sorter?</a>`;
  const timeStr = `This sorter took ${msToReadableTime(timeTaken)}. <a href="${location.protocol}//${sorterURL}">Do another sorter?</a>`;
  const imgRes = (char, num) => {
    const charName = reduceTextWidth(char.name, 'Arial 12px', 160);
    const charTooltip = char.name !== charName ? char.name : '';
    // return `<div class="result image"><div class="left"><span>${num}</span></div><div class="right"><img src="${char.img}"><div><span title="${charTooltip}">${charName}</span></div></div></div>`;
